...
export const Login: React.FC = () => {
  const { login, googleLogin, demoLogin, isLoading } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      // Navigate to /dashboard for existing users as requested
      navigate('/dashboard');
    } else {
      setFormError(result.error || 'Invalid credentials. Please check and try again.');
    }
  };
    ...
    <div className="min-h-screen bg-[#07090e] flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden selection:bg-blue-500/30 selection:text-blue-200">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md px-4">
        ...
        <div className="text-center space-y-2 mb-8">
          ...
          >
            Log in to your LeadPilot AI account.
          </p>
        </div>
        ...
        <div className="rounded-2xl bg-[#0b0f1a] border border-blue-500/30 p-6 sm:p-8 shadow-2xl shadow-blue-950/40 ring-1 ring-white/10 relative">
          ...
          {forgotSuccess && (
            <div className="mb-5 p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-300 flex items-center gap-2">
              <CheckCircle2 className="w-4 h-4 shrink-0" />
              <span>Password reset instructions sent if email exists.</span>
            </div>
          )}
          ...
          <button
            ...
            disabled={isSubmitting}
            className="w-full py-3 px-4 rounded-xl bg-slate-900 hover:bg-slate-850 text-slate-200 border border-slate-700/80 hover:border-slate-600 text-xs font-semibold flex items-center justify-center gap-3 transition-colors disabled:opacity-60 mb-3"
          >
          ...
          <button
            ...
            disabled={isSubmitting}
            className="w-full py-2.5 px-4 rounded-xl bg-blue-950/40 hover:bg-blue-900/40 text-blue-300 border border-blue-800/50 text-xs font-semibold flex items-center justify-center gap-2 transition-colors"
          >
            <Sparkles className="w-3.5 h-3.5 text-blue-400" />
            <span>1-Click Demo Login (Dr. Sarah — Dental Practice)</span>
          </button>
